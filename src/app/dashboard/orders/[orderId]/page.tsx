...
export default function OrderDetailsPage() {
  ...
  // For admins, the shopId might be in the query params.
  ...
  // Determine the shopId based on the user's role and query params
  const shopId = userData?.role === 'admin' ? adminSelectedShopId : userData?.shopId;

  const orderDocRef = useMemoFirebase(() => {
    if (!orderId || !shopId) return null;
    return doc(firestore, `shops/${shopId}/orders`, orderId);
  }, [firestore, orderId, shopId]);

  const { data: order, isLoading: isOrderLoading } = useDoc<OrderDetails>(orderDocRef);

  const isLoading = isUserDataLoading || isOrderLoading;
     ...
     return (
      <Card>
        <CardHeader>
            ...
            <CardDescription>The requested order could not be found or you do not have permission to view it.</CardDescription>
        </CardHeader>
        <CardContent>
